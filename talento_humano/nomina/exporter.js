...
const EXPORTER = (() => {
    ...
    function exportarPDF(liquidaciones, periodo) {
        ...
        liquidaciones.forEach(liq => {
            doc.addPage();
            dibujarRecibo(doc, liq, periStr, margen, ancho);
        });

        doc.save(`nomina_${periStr.replace(/\s+/g, '_')}.pdf`);
    }

    function exportarReciboPDF(liq, periodo) {
        if (typeof jspdf === 'undefined') {
            alert('La biblioteca jsPDF no está disponible. Verifica tu conexión a Internet.');
            return;
        }
        ...
        const nombreLimpio = liq.empleado.nombre.replace(/\s+/g, '_');
        const qStr = periodo.quincena === '1' ? '1Q' : '2Q';
        doc.save(`Comprobante_${nombreLimpio}_${qStr}_${periStr.split(' de ')[1].replace(/\s+/g, '_')}.pdf`);
    }

    // Helper privado para dibujar el cuerpo del recibo
    function dibujarRecibo(doc, liq, periStr, margen, ancho) {
        // Cabecera empleado
        doc.setFillColor(30, 41, 59);
    ...
    return { exportarPDF, exportarReciboPDF, exportarExcel, exportarParafiscalesExcel, exportarPrestacionesExcel };
