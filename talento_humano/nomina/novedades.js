...
const NOMINA = (() => {
    ...
    const AUX_TRANSPORTE = 249_095; // Confirmado como 249,095 según contexto legal 2026
    ...
    function valorDia(salarioMensual) {
        // Fórmula requerida: (Salario / 2) / 15
        return (salarioMensual / 2) / 15;
    }

    function valorHora(salarioMensual) {
        // Basado en 8 horas diarias sobre el valor día calculado
        return valorDia(salarioMensual) / 8;
    }
    ...
    function calcularNovedad(novedad, salarioMensual) {
        ...
        switch (novedad.tipo) {
            ...
            case 'hora_no_laborada': {
                const horas = Number(novedad.valor) || 0;
                // Se descuenta al mismo valor de la hora extra diurna (1.15x) según imagen del usuario
                deduccion = horas * vHora * (1 + RECARGOS.hora_extra_diurna);
                descripcion = `Horas no laboradas (${horas.toFixed(2)} h)`;
                break;
            }
            case 'hora_viaje': {
                const horas = Number(novedad.valor) || 0;
                adicion = horas * vHora * 0.5; // Tarifa 0.5x
                descripcion = `Horas de viaje (${horas.toFixed(2)} h)`;
                break;
            }
            default:
    ...
    function liquidar(empleado) {
        ...
        const pensionQ = SMMLV * PENSION_EMP / 2;
        ...
        // Parafiscales con nivel ARL del empleado (FORZADO SOBRE SMMLV)
        const nivelARL = empleado.nivelARL || 'I';
        const paraMes = calcularParafiscalesMes(SMMLV, nivelARL);
        const paraQ = {
            nivelARL,
        ...
        // Provisión de prestaciones sociales (quincenal) - FORZADO SOBRE SMMLV
        const auxMes = tieneAuxTransporte(SMMLV) ? AUX_TRANSPORTE : 0;
        const prestMes = calcularPrestacionesMes(SMMLV, auxMes);
        const prestQ = {
    ...
    return {
        ...
        TIPOS_NOVEDAD: [
            ...
            { value: 'hora_no_laborada', label: 'Horas no laboradas', unidad: 'horas', efecto: 'deducción' },
            { value: 'hora_viaje', label: 'Horas de viaje', unidad: 'horas', efecto: 'adición' },
        ],
    };
})();
