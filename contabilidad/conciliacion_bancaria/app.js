...
const STORAGE_KEY = 'conciliacion_bancaria_state';
...
const state = {
    cuentas: [
        { id: 'c1', nombre: 'BBVA Principal', banco: 'BBVA' }
    ],
    cuentaActiva: 'c1',
    archivoBanco: null,
    archivoHelisa: null,
...
function renderCuentas() {
    ...
    state.cuentas.forEach(c => {
        ...
        btn.innerHTML = `<span class="tab-bank-icon">${c.banco === 'BBVA' ? '🏛️' : '🏦'}</span> ${c.nombre}`;
        btn.onclick = () => {
...
function getCuentaActiva() {
    let cuenta = state.cuentas.find(c => c.id === state.cuentaActiva);
    // Si por alguna razón no existe (ej: caché de versión anterior), devolver la primera
    if (!cuenta && state.cuentas.length > 0) {
        cuenta = state.cuentas[0];
        state.cuentaActiva = cuenta.id;
    }
    return cuenta;
}
...
async function doConciliar() {
    ...
    if (!cuenta) {
        showToast('❌ No se encontró la cuenta seleccionada.', 'error');
        return;
    }
    ...
    try {
        ...
        // Parsear
        let movsBanco;
        if (cuenta.banco === 'Banco de Occidente' || cuenta.banco === 'Bancolombia') {
            movsBanco = parseBancolombia(wbBanco);
        } else if (cuenta.banco === 'BBVA' || cuenta.banco === 'Davivienda') {
            movsBanco = parseBBVA(wbBanco);
        } else {
            throw new Error('Banco no reconocido: ' + cuenta.banco);
        }
...
function readWorkbook(file) {
    if (!file) return Promise.reject(new Error('No se ha seleccionado ningún archivo.'));
    if (file.size === 0) return Promise.reject(new Error(`El archivo "${file.name}" está vacío.`));

    return new Promise((resolve, reject) => {
        // Envoltura para capturar el error detallado de la primera estrategia
        let firstRelevantError = null;

        // Estrategia Principal: ArrayBuffer (XLSX, XLS binario)
        const tryArray = () => new Promise((res, rej) => {
            ...
            r.onload = e => {
                try {
                    ...
                    if (!wb || !wb.SheetNames || wb.SheetNames.length === 0) throw new Error('El archivo no contiene hojas válidas.');
                    res(wb);
                } catch (err) { rej(err); }
            };
            r.onerror = () => rej(new Error('Error de E/S al leer el archivo binario.'));
            r.readAsArrayBuffer(file);
        });

        // Estrategia Alternativa: Texto (CSV, TXT, HTML tablas)
        const tryText = (encoding = 'UTF-8') => new Promise((res, rej) => {
            const r = new FileReader();
            r.onload = e => {
                try {
                    const text = e.target.result;
                    if (text.includes('<html') || text.includes('<table')) {
                        res(XLSX.read(text, { type: 'string' }));
                    } else {
                        const sep = text.includes('\t') ? '\t' : (text.includes(';') ? ';' : ',');
                        res(XLSX.read(text, { type: 'string', FS: sep }));
                    }
                } catch (err) { rej(err); }
            };
            r.onerror = () => rej(new Error('Error de E/S al leer como texto.'));
        ...
        let promise;
        if (ext === 'csv' || ext === 'txt') {
            promise = tryText('UTF-8')
                .catch(err => { firstRelevantError = err; return tryText('ISO-8859-1'); })
                .catch(() => tryArray());
        } else {
            promise = tryArray()
                .catch(err => { 
                    firstRelevantError = err; 
                    // Si falla el binario en un .xls/.xlsx, podría ser un HTML camuflado
                    return tryText('UTF-8'); 
                })
                .catch(() => tryText('ISO-8859-1'));
        }

        promise
            .then(resolve)
            .catch(lastErr => {
                const finalErr = firstRelevantError || lastErr;
                const msg = finalErr.message || 'Error desconocido';
                
                let help = 'Verifique que el archivo no esté protegido con contraseña y que sea un Excel válido.';
                if (msg.includes('password') || msg.includes('decrypt')) help = 'El archivo parece estar protegido con contraseña. Por favor, guarde una copia sin protección.';
                if (msg.includes('Corrupted')) help = 'El archivo parece estar dañado.';

                reject(new Error(`No se pudo procesar "${file.name}". Motivo: ${msg}. ${help}`));
            });
    });
}
...
function renderResultados(result, cuenta) {
    ...
    const pct = s.porcentajeCruce || 0;
    const resPct = document.getElementById('res-pct');
    resPct.textContent = pct + '%';
    
    // Color dinámico según porcentaje
    const cardPct = resPct.closest('.metric-card');
    cardPct.classList.remove('card-ok', 'card-warn', 'card-error');
    if (pct >= 90) cardPct.classList.add('card-ok');
    else if (pct >= 70) cardPct.classList.add('card-warn');
    else cardPct.classList.add('card-error');
