...
function parseBancolombia(workbook) {
    ...
    // Intentar encontrar encabezados con débito+crédito o con importe/valor
    let found = findHeaderRow(sheet, ['fecha', 'débito', 'crédito']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'debito', 'credito']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'importe']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'valor', 'saldo']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'monto']);
    if (!found) throw new Error(
        'Bancolombia: no se encontró la fila de encabezados. ' +
...
function parseBBVA(workbook) {
    ...
    let found = findHeaderRow(sheet, ['fecha', 'importe']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'débito', 'crédito']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'debito', 'credito']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'monto']);
    if (!found) found = findHeaderRow(sheet, ['fecha', 'valor']);
    if (!found) throw new Error(
        'BBVA: no se encontró la fila de encabezados. ' +
        'El archivo debe tener columnas de Fecha e Importe o Débito/Crédito.'
    );
...
function parseHelisa(workbook) {
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    if (!sheet || !sheet['!ref']) throw new Error('Helisa: hoja de cálculo vacía o no reconocida.');

    const range = XLSX.utils.decode_range(sheet['!ref']);
    const result = [];

    // ── Paso 1: convertir todas las celdas a una matriz 2D ──────
    const matrix = [];
