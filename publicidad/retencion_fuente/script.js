document.addEventListener('DOMContentLoaded', () => {
    ...
    const BASE_STORAGE_KEY = 'reten_fuente_v2_publicidad_';
    ...
    const invConceptSelect = document.getElementById('invConcept');
    ...
    const loadData = () => {
        ...
        const globalRaw = localStorage.getItem(CONFIG_KEY);
        ...
        const prov = localStorage.getItem(PROVIDERS_KEY);
    ...
    const saveData = () => {
        // Guardar configuración global y proveedores
        localStorage.setItem(CONFIG_KEY, JSON.stringify(config));
        localStorage.setItem(PROVIDERS_KEY, JSON.stringify(providerRegistry));
    ...
    const handleProviderSelection = (name) => {
        ...
        if (p) {
            invConceptSelect.value = p.conceptId;
    ...
    const renderProvidersRegistry = () => {
        ...
        sortedNames.forEach(name => {
            ...
            tr.innerHTML = `
                ...
                <td style="font-size: 0.85rem;">${concept?.name || p.conceptId}</td>
    ...
    const initConceptDropdown = () => {
        invConceptSelect.innerHTML = CONCEPTS.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
    };
    ...
    const render = () => {
        ...
        invoices.forEach(inv => {
            ...
            // Si la retención es 0 (ej: por debajo de base) y no es concepto manual,
            ...
            if (inv.personType === 'PN') {
                aggregates[inv.conceptId].basePN += effectiveBase;
                aggregates[inv.conceptId].retPN += ret;
            } else {
                aggregates[inv.conceptId].basePJ += effectiveBase;
                aggregates[inv.conceptId].retPJ += ret;
            }

            aggregates[inv.conceptId].total += ret;
        });
        ...
        CONCEPTS.forEach(c => {
            ...
            tr.innerHTML = `
                ...
                <td class="num-cell pn-cell">$ ${Math.round(agg.basePN).toLocaleString()}</td>
                <td class="num-cell pj-cell">$ ${Math.round(agg.basePJ).toLocaleString()}</td>
                <td class="num-cell retencion-cell">$ ${Math.round(agg.total).toLocaleString()}</td>
                <td class="num-cell pn-cell">${agg.retPN > 0 ? '$ ' + Math.round(agg.retPN).toLocaleString() : '-'}</td>
                <td class="num-cell pj-cell">${agg.retPJ > 0 ? '$ ' + Math.round(agg.retPJ).toLocaleString() : '-'}</td>
            `;
        ...
        totalRetenerEl.textContent = `$ ${Math.round(totalRet - excess).toLocaleString()}`;
        totalBaseEl.textContent = `$ ${Math.round(totalBase).toLocaleString()}`;
    ...
    const renderHistory = (filterConceptId = null) => {
        ...
        const filteredInvoices = filterConceptId
            ? invoices.filter(inv => inv.conceptId === filterConceptId)
            : invoices;
        ...
        filteredInvoices.forEach((inv, originalIdx) => {
            ...
            tr.innerHTML = `
                ...
                <td class="concept-edit-cell" style="cursor: pointer;" onclick="window.startInlineEdit(event, ${realIdx})">
                    <div style="color: var(--primary); text-decoration: underline dotted;">${concept?.name || 'Otro'}</div>
                ...
                <td class="num-cell">$ ${Math.round(concept.isIVA ? (inv.iva !== null && inv.iva !== undefined ? inv.iva : inv.base * 0.19) : inv.base).toLocaleString()}</td>
                <td class="num-cell" style="font-weight: 600; color: var(--primary);">$ ${calculateItem(inv, concept).toLocaleString()}</td>
    ...
    window.startInlineEdit = (event, index) => {
        ...
        const currentConceptId = invoices[index].conceptId;
        ...
        CONCEPTS.forEach(c => {
            ...
            opt.selected = (c.id === currentConceptId);
            select.appendChild(opt);
        });

        select.onchange = () => {
            invoices[index].conceptId = select.value;
            saveData();
            render();
            // Mantener el modal abierto si estamos filtrando
            const currentTitle = document.querySelector('#modalHistory h2').textContent;
            if (currentTitle.startsWith('Facturas:')) {
                renderHistory(select.value); // Re-filtrar por el nuevo concepto si se prefiere, o dejar el actual
            } else {
                renderHistory();
            }
        };
        ...
        select.onblur = () => {
            const concept = CONCEPTS.find(c => c.id === invoices[index].conceptId);
            cell.innerHTML = concept?.name || 'Otro';
        };
    ...
    window.editInvoice = (index) => {
        ...
        document.getElementById('invConcept').value = inv.conceptId;
    ...
    const processExcelInvoices = (data) => {
        ...
        data.forEach((row) => {
            ...
            const desc = normalizedRow['nombre emisor'] || normalizedRow['nombre'] || normalizedRow['tercero'] || normalizedRow['descripcion'] || normalizedRow['detalle'] || row[keys[0]] || 'Factura Importada';
            const nit = normalizedRow['nit emisor'] || normalizedRow['nit'] || normalizedRow['identificacion'] || '';
            const fullDesc = nit ? `${nit} - ${desc}` : desc;

            let rawBase = normalizedRow['base'] || normalizedRow['subtotal'] || normalizedRow['valor total'] || normalizedRow['total'] || normalizedRow['valor'] || normalizedRow['monto'];
            if (rawBase === undefined) {
                for (let k in row) {
                    const val = row[k];
                    const numVal = cleanNumber(val);
                    if (!isNaN(numVal) && numVal !== 0 && typeof val !== 'boolean') {
                        rawBase = val;
                        break;
                    }
                }
            ...
            const conceptText = (normalizedRow['concepto'] || normalizedRow['tipo'] || normalizedRow['tipo de documento'] || '').toString().toLowerCase();

            const personText = (normalizedRow['persona'] || normalizedRow['regimen'] || normalizedRow['tipo persona'] || '').toString().toUpperCase();
            const personType = personText.includes('NATURAL') || personText === 'PN' ? 'PN' : 'PJ';

            // Detección de IVA
            let iva = normalizedRow['iva'] || normalizedRow['valor iva'] || normalizedRow['impuesto'] || normalizedRow['impuestos'];
    ...
    invoiceForm.onsubmit = (e) => {
        ...
        const inv = {
            ...
            conceptId: invConceptSelect.value,
            base: parseFloat(document.getElementById('invBase').value),
            ...
            iva: (index !== -1 && invoices[index].base === parseFloat(document.getElementById('invBase').value)) ? invoices[index].iva : null
        };
    ...
    const exportToExcel = () => {
        ...
        const excelData = invoices.map(inv => {
            ...
            // En Excel normalmente se quiere ver todo, lo dejamos.
            ...
            return {
                ...
                'Concepto': concept ? concept.name : inv.conceptId,
                'Tipo': inv.personType,
